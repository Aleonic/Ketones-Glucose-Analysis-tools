...
createGroup <- function(df, gname, type, low, high, xlsx_value) {
  ...
  limitDB <- filter(df, as.numeric(Day_on_PKT) <= as.numeric(high))
  limitDB <- filter(limitDB, as.numeric(Day_on_PKT) >= as.numeric(low))
  
  patients <- limitDB %>%
    select(Patient_Name) %>%
    distinct(Patient_Name)
  
  cat(gname, ":\n\nNumber of Patients in group:", nrow(patients), "\n")
  
  group_2OM <- limitDB %>% count(Patient_Name) %>% filter(n > 1)
  
  patientDB <- left_join(group_2OM, df, by=c("Patient_Name" = "Patient_Name"))
  
  patientDB <- patientDB %>% filter(!is.na(Day_on_PKT)) %>% filter(Day_on_PKT != 0)
  
  plotData(limitDB, gname, "Samples", type, low, high)
  plotData(patientDB, gname, "Patients", type, low, high)
  
  writeToExcel(limitDB, gname, "Samples", xlsx_value)
  writeToExcel(patientDB, gname, "Patients", xlsx_value)
}

plasmaFilter <- function(plasma) {
